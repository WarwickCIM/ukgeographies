...
#'
#' Metadata about a subset of the geographical boundaries from
#' [ONS'geoportal](https://geoportal.statistics.gov.uk/) that can be downloaded
#' by `{ukgeographies}`. These boundaries have been retrieved on
#' `r as.Date(file.mtime("data/ons_boundaries.rda"))` from the services
#' provided by [ONS' geoportal API](https://services1.arcgis.com/ESMARspQHYMw9BZ9/arcgis/rest/services/)
#' and automatically categorised from their URLS, to match Geoportal's menus and
#' datasets.
#'
#' @format A data frame with `r nrow(ons_boundaries)` rows and
#' `r ncol(ons_boundaries)` columns:
#' \describe{
...
#'   \item{url_metadata}{URL to a webservice's metadata in a json format}
#'
#' }
#'
